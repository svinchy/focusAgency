...
const renderService = (index, { updateContent = true, immediate = false } = {}) => {
  ...
  if (title) {
    ...
    if (title.textContent !== nextTitle || immediate) {
      ...
      if (immediate) { title.textContent = nextTitle; title.classList.remove('swap-out', 'swap-in'); }
      else {
  ...
  if (description) {
    ...
    if (description.textContent !== nextDescription || immediate) {
      ...
      if (immediate) { description.textContent = nextDescription; description.classList.remove('swap-out', 'swap-in'); }
      else {
  ...
  if (list && updateContent) {
    ...
    requestAnimationFrame(() => { service.classList.remove('updating'); });
  }
};
...
export const scrollSwitcher = function scrollSwitcher() {
  const content = content_data();
  const service = document.querySelector('.service');
  ...
  const holdDotsVisible = () => { keepDotsVisibleUntilManualScroll = true; };
  const releaseDotsVisibilityHold = () => { keepDotsVisibleUntilManualScroll = false; };
  ...
  const resetManualServiceTouch = () => { manualServiceScrollEl = null; manualServiceTouchStartY = 0; manualServiceScrollStartTop = 0; };
  ...
  const onServiceTouchStart = (e) => {
    ...
    if (!touch || !scrollEl) { resetManualServiceTouch(); return; }
  ...
  const onDocumentTouchMoveLock = (e) => {
    ...
    if (canScrollInsideServiceContent(e.target, deltaY)) { touchStartY = touch.clientY; return; }
    e.preventDefault();
    touchStartY = touch.clientY;
  };
  ...
  const applyPageScrollLock = () => {
    ...
    if (!body || !html) return;
    if (typeof window.syncDocumentScrollLock === 'function') { window.syncDocumentScrollLock(); return; }
  ...
  const applyServicesSectionHeight = () => {
    ...
    if (isStaticLayout) { servicesSection.style.removeProperty('min-height'); return; }
  ...
  const scrollPageToServiceIndex = (index) => {
    if (isStaticLayout) { setActiveIndex(index, { scrollImage: true, forceScroll: true }); return null; }
    ...
    if (typeof window.__smoothScrollTo === 'function') { window.__smoothScrollTo(targetTop); return targetTop; }
  ...
  const updateDotsPosition = () => {
    ...
    if (isStaticLayout) { dotsWrap.style.removeProperty('left'); dotsWrap.style.removeProperty('right'); dotsWrap.style.removeProperty('top'); dotsWrap.style.removeProperty('bottom'); dotsWrap.style.removeProperty('transform'); return; }
  ...
  const setActiveIndex = (index, { scrollImage = true, forceScroll = false, imageBehavior = 'smooth', immediateText = false } = {}) => {
    ...
    else { activeIndex = index; preloadServiceImagesAround(index); }
    if (shouldRender) {
    ...
    if (imagesWrapEl && scrollImage && (!isSameIndex || forceScroll)) {
      imagesWrapEl.scrollTo({ top: getImageScrollTarget(imagesWrapEl, index), behavior: imageBehavior });
    }
  ...
  if (service) {
    service.addEventListener('click', (e) => {
      ...
      if (isActive) {
        ...
        setActiveIndex(activeIndex, { scrollImage: true, forceScroll: true, imageBehavior: isStaticLayout ? 'auto' : 'smooth' });
      }
    });
  }
  ...
  dots.forEach((dot, i) => {
    dot.addEventListener('click', (e) => {
      e.preventDefault();
      holdDotsVisible();
      if (isStaticLayout) { pendingDotIndex = null; pendingDotTop = null; setActiveIndex(i, { scrollImage: true, forceScroll: true }); return; }
      if (service && service.classList.contains('active')) { pendingDotIndex = null; pendingDotTop = null; setActiveIndex(i, { scrollImage: true, forceScroll: true }); return; }
      pendingDotIndex = i;
      setActiveIndex(i, { scrollImage: false });
      pendingDotTop = scrollPageToServiceIndex(i);
      const scrollY = window.scrollY || window.pageYOffset;
      if (pendingDotTop === null || Math.abs(scrollY - pendingDotTop) <= 2) { pendingDotIndex = null; pendingDotTop = null; }
    });
  });
  ...
  const handleScroll = () => {
    ...
    updateDotsPosition();
    if (isStaticLayout) { if (dotsWrap) dotsWrap.classList.add('is-visible'); return; }
    const contentRect = contentWrap.getBoundingClientRect();
  ...
  if (imagesWrap) {
    ...
    imagesWrap.addEventListener('wheel', (e) => {
      ...
      else window.scrollBy({ top: e.deltaY, left: 0, behavior: 'auto' });
    }, { passive: false });
  }

  if (dots.length > 0) setActiveIndex(0);
  if (document.body) { syncServiceScrollLock(false); applyPageScrollLock(); }
  updateDotsPosition();
  onScroll();
};
