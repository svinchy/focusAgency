export const loadingIntro = function loadingIntro() {
  ...
  window.setTimeout(() => { body.classList.add('intro-reveal'); }, 1000);
  window.setTimeout(() => { body.classList.add('intro-corners'); }, 1400);

  const moveStart = 2600;
  window.setTimeout(() => { body.classList.add('intro-corners-move'); }, moveStart);

  const moveDuration = 3000;
  const fadeStart = moveStart + Math.round(moveDuration * 0.1);
  window.setTimeout(() => { body.classList.add('intro-fade'); }, fadeStart);
  ...
  window.setTimeout(() => { startGlobeZoom(); }, globeZoomStart);

  const bannerStart = globeZoomStart + Math.round(globeZoomDuration * 0.2);
  window.setTimeout(() => { startBannerReveal(); }, bannerStart);

  const introEnd = Math.max(moveEnd, globeZoomStart + globeZoomDuration) + 120;
  window.setTimeout(() => { finishIntro(); startBannerReveal(); }, introEnd);
